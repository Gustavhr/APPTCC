...
public class UserDAO extends SQLiteOpenHelper {
    ...
    private static String nome="Usuario.db";
    ...
    public void onCreate(SQLiteDatabase db){
        String str = "CREATE TABLE USER (id INTERGER PRIMARY KEY  , usuario TEXT UNIQUE , senha TEXT , tipousuario TEXT,nome TEXT,email TEXT);";
        db.execSQL(str);
    ...
    {
       Usuario usuario;
        Cursor cursor = getReadableDatabase().rawQuery("SELECT * FROM USER WHERE usuario =?" , new String[] {login});
        usuario = new Usuario();
        if(cursor.getCount()>0) {
            ...
            while (cursor.moveToNext()) {
                ...
                usuario.setLogin(cursor.getString(cursor.getColumnIndex("usuario")));
                usuario.setTipo(cursor.getString(cursor.getColumnIndex("tipousuario")));
